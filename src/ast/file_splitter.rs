...
impl AstBasedFileSplitter {
    ...
    pub async fn vectorization_split(
        ...
        files_markup.insert(path_str.to_string(), Arc::new(crate::scratchpads::chat_utils_rag::File { markup: ast_markup, cpath: doc.path, cpath_symmetry_breaker: 0.0 }));
            ...
            .symbol_info_struct())
        {
            if symbol.symbol_type == SymbolType::StructDeclaration {
                ...
                // if let Some(gcx) = global_context.upgrade() {
                    // let res = crate::scratchpads::chat_utils_rag::postprocess_rag_stage_3_6(
                    //     gcx.clone(),
                    //     messages,
                    //     files_markup,
                    //     true).await;
                ...
                let (mut lines_in_files, mut lines_by_useful) = crate::scratchpads::chat_utils_rag::postprocess_rag_stage_3_6(
                    global_context.upgrade().unwrap(), messages, files_markup, close_small_gaps,
                ).await;
                info!("lines_in_files: {:?}", lines_in_files);
                info!("lines_by_useful: {:?}", lines_by_useful);

                let res = crate::scratchpads::chat_utils_rag::postprocess_rag_stage_7_9(&mut lines_in_files, &mut lines_by_useful, tokenizer, tokens_limit, single_file_mode).await;

                // let res = crate::scratchpads::chat_utils_rag::postprocess_rag_stage_7_9(global_context.upgrade().unwrap(), messages, files_markup, tokenizer, tokens_limit, single_file_mode, close_small_gaps).await;
                info!("{:?}", res);

                // let res = crate::scratchpads::chat_utils_rag::postprocess_at_results2(gcx.clone(), messages, tokenizer.clone(), tokens_limit, single_file_mode).await;
                if let Some(first) = res.first() {
                    // info!("{} content was:\n{}", symbol.name, content);
                    let content = first.file_content.clone();
