...
const App = (props: Partial<ComboBoxProps>) => {
  ...
  function fakeRequestCommands(query: string, cursor: number): void {
    // console.log({ query, cursor });
    if (query[cursor - 1] === "@") {
      setCommands({
        completions: ["@file", "@workspace"],
...
describe("ComboBox", () => {
  afterEach(cleanup);
  test.only("type @ and select command and arguments by clicking", async () => {
    ...
    // await user.type(app.getByRole("combobox"), "hello");
    // await user.type(textarea, "hello", {
    //   initialSelectionStart: textarea.value.length,
    // });
  ...
  test("deleting while typing a command", async () => {
    const { user, ...app } = render(<App />);
    const textarea = app.getByRole("combobox");
    await user.type(textarea, "@f{Tab}{Space}f{Tab}");
