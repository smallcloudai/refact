...
class GptChatWithFunctions(ascratch.AsyncScratchpad):
    ...
    async def _run_function(self) -> AsyncIterator[
        ...
        params: Dict[str, str] = json.loads(self._function_call['arguments'])
        raw_function_call = json.dumps({'name': name, 'arguments': params})
    ...
    async def _call_openai_api_based_on_stored_messages(
        ...
        try:
            while True:
                ...
                if delta_function := delta.get('function_call'):
                    self._on_function = True
                    self._function_call.setdefault('name', delta_function.get('name'))
                    self._function_call.setdefault('arguments', '')
                    self._function_call['arguments'] += delta_function['arguments']
                ...
                if self.finish_reason:
                    break
                if (tokens % self._accumulate_n_streaming_chunks == 0) and not self._on_function:
    ...
    async def completion(self) -> AsyncIterator[
        ...
        if self._on_function:
            self._on_function = False
            async for res in self._call_openai_api_based_on_stored_messages(use_functions=False):
                yield res
