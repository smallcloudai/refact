...
function getChildren(
  ...
  const [_rest, siblings] = partitionChildrenAndSiblings(head, tail);

  const children = [head, ...siblings].map((m) => {
    return {
      message: m,
      children: getChildren(m, _rest, memo),
    };
  });
...
function partitionChildrenAndSiblings(
  node: CMessage,
  messages: CMessage[],
): [CMessage[], CMessage[]] {
  const [nodes, siblings] = partition(
    messages,
    (m) => m.cmessage_prev_alt === node.cmessage_num,
  );
