...
async fn get_agents_md(gcx: &Arc<ARwLock<GlobalContext>>) -> Result<ChatMessage, String> {
    ...
    let mut files_in_dir = tokio::fs::read_dir(&act_proj_path)
        .await
    ...
    let found_agents_md = {
        ...
        while let Ok(Some(entry)) = files_in_dir.next_entry().await {
            ...
            if file_name.to_lowercase() == "agents.md" {
                found_agents_md.push(file_name);
            }
        }
    ...
    for file_name in found_agents_md {
        let agents_md_path = act_proj_path.join(&file_name);

        let content = tokio::fs::read_to_string(&agents_md_path)
            .await
