...
use crate::{
    ...
    files_correction::{correct_to_nearest_dir_path, get_project_dirs, canonicalize_normalized_path, preprocess_path_for_normalization},
...
async fn parse_args(
    ...
    let related_files = match args.get("related_files") {
        Some(Value::String(s)) => {
            let mut paths = vec![];
            for s in s.split(",") {
                let s_raw = s.trim().to_string();
                let candidates_file = file_repair_candidates(gcx.clone(), &s_raw, 3, false).await;
                if let Ok(f) = return_one_candidate_or_a_good_error(gcx.clone(), &s_raw, &candidates_file, &get_project_dirs(gcx.clone()).await, false).await {
                    paths.push(canonicalize_normalized_path(PathBuf::from(preprocess_path_for_normalization(f.trim().to_string()))));
                } else {
...
impl Tool for ToolDebugScript {
    ...
    async fn tool_execute(
        ...
        let mut additional_files_content = String::new();
        for path in &parsed_args.related_files {
            if let Ok(content) = std::fs::read_to_string(path) {
                let file_name = path.to_string_lossy();
                additional_files_content.push_str(&format!(
                    "\n\nRelated file: {}\n```python\n{}\n```\n",
                    file_name,
                    content
                ));
                tracing::info!(
                    ...
                    "Added related file to debugging context"
                );
            } else {
                tracing::warn!(
                    target: "debug_script",
                    file = path.to_string_lossy().to_string(),
                    "Failed to read related file"
                );
            }
        }
            
        let debug_model = "refact/claude-sonnet-4".to_string();
        ...
        ToolDesc {
            ...
            parameters: vec![
                ...
                ToolParam {
                    ...
                    description: "Comma-separated list of additional related files that might be helpful for debugging".into(),
                    param_type: "string".into(),
                },
            ],
